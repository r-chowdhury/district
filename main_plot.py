...
def PlotAllVoronoi(C, A, assignment, bounded_regions, bbox, output):
    ...
    for j in range(len(A)):
        f.write(
            str(A[j][0]) + " " + str(A[j][1]) + " " + str(colors[assignment[j]]) + "\n"
        )
...
def plot_helperVoronoi_fromfile(input_filename, outputfile):
    C_3D, A, assign_pairs, bbox = vb.Parse(input_filename)  # changed "box" to "bbox" -Neal
    C = [[p[0], p[1]] for p in C_3D]
    regions = vb.power_cells(C_3D, bbox)
    PlotAllVoronoi(C, A, assign_pairs, regions, bbox, outputfile)
...
def ParseGNUplot(filename):
    ...
    for i in range(nb_centers + nb_clients + 1, len(lines)):
        ...
        points = [
            [
                float(points_unsplit[j].split(",")[0]),
...
def GNUplot(
    ...
    if block_data_filename != "":
        for block in census_block_file.read(block_data_filename):
            blockid = block.ID
    ...
    f.write(
        "set xrange ["
        + str(bbox[0][0] - offset_x)
        + ":"
        + str(bbox[1][0] + offset_x)
        + "]\n"
    )
    f.write(
        "set yrange ["
        + str(bbox[0][1] - offset_y)
        + ":"
        + str(bbox[1][1] + offset_y)
        + "]\n"
    )
    f.write("set key off\n")
