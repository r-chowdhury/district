...
def plot_polygons(polygons, bbox=None):
    f, ax = plt.subplots()
    minpt, maxpt = find_bbox([pt for p in polygons for pt in p.exterior.coords]) if bbox == None else bbox
    if bbox == None: print("bounding box:", (minpt, maxpt))
...
def plot_polygon(gon): plot_polygons([gon])
